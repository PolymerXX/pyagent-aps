...
"""
    ...
    async def handle_new_order(
        ...
        if existing_result:
            orders = [Order(**a) for a in existing_result.assignments]
            orders.append(order)
        else:
            orders = [order]
    ...
    async def handle_machine_down(
        ...
        affected = [a.get("order_id") for a in result.get("assignments", [])]
    ...
    async def analyze_and_adjust(
        ...
        for violation in validation.constraint_violations:
            if violation.get("type") == "due_date":
                return await self.handle_order_change(
                    order_id=violation.get("order_id"),
